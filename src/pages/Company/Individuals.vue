<template>
  <q-page padding>
    <q-card>
      <q-card-section class="text-bold bg-grey-2">Add New Individual</q-card-section>
      <q-item>
        <q-item-section><q-input v-model="newIndividual.name" outlined label="Individual Name" /></q-item-section>
        <q-item-section side><q-btn icon="forward" flat dense color="primary" @click="addNewIndividual" /></q-item-section>
      </q-item>
      <q-card-section class="text-bold bg-grey-2 flex items-center">Existing Individuals<q-space /><q-chip :label="Individuals ?. length || 0" class="q-ma-none" /></q-card-section>
      <q-list separator>
        <q-item v-for="Individual in Individuals">
          ...
          <q-item-section side><q-btn dense icon="login" flat color="primary" @click="manageLogin(Individual)" /></q-item-section>
    ...
    <q-dialog persistent :model-value="individualLogin !== null" @before-hide="individualLogin = null"><IndividualUsersManage class="full-width" :company_id="props.company_id" :individual_id="individualLogin" /></q-dialog>
