  ...
  <q-page padding>
    <q-card>
      <q-card-section class="q-gutter-y-xs">
        ...
        <q-btn label="Add Manager" color="primary" @click="addUser" />
      </q-card-section>
      <q-card-section class="text-bold bg-grey-1">Managers</q-card-section>
      <q-list>
...
const users = computed(() => filter(userStore.Users,user => user.role === 'Manager'))

function addUser(){
  loading.value = true
  userStore.addUser({ name:name.value,email:email.value,password:password.value },'Manager').then(() => {
    name.value = ''; email.value = ''; password.value = '';
  }).then(() => loading.value = false)
}

function manage(user){ _manage.value = user.id }
</script>
